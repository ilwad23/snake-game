...
# SNAKE
SPOTS_FOR_SNAKE = 20
snake_head = pygame.Rect((HALFW, HALFH), (WIDTH/SPOTS_FOR_SNAKE, HEIGHT/SPOTS_FOR_SNAKE))
...
def add_body():
    ...
    new_part = pygame.Rect((snake_head.w, snake_head.h), (WIDTH/SPOTS_FOR_SNAKE, HEIGHT/SPOTS_FOR_SNAKE)) 
...
def controls(vel,move_snake):
    ...
    if key[pygame.K_LEFT] and move_snake[1] != 'right':move_snake = [[-abs(vel),0],'left']
    if key[pygame.K_RIGHT] and move_snake[1] != 'left':move_snake = [[abs(vel),0], 'right']
    if key[pygame.K_UP] and move_snake[1] != 'down':move_snake = [[0, -abs(vel)], 'up']
    if key[pygame.K_DOWN] and move_snake[1] != 'up':move_snake = [[0,abs(vel)], 'down']
...
def handle_move_for_snake(vel, move_snake, still_out):
    ...
    if out_of_screen and still_out == 0:
        still_out =1; 
        if (move_snake[1] == 'right'): body_arr[last].x = 1
        if (move_snake[1] == 'left'):  body_arr[last].x = WIDTH-1
        if (move_snake[1] == 'up'): body_arr[last].y = HEIGHT-body_arr[last].height
        if (move_snake[1] == 'down'): body_arr[last].y = 1
...
def display_on_screen(move_snake):
    ...
    for i, p in enumerate(body_arr):    
        ...
        if i != 0:
            p.update((body_arr[i-1].x-snake_head.w, body_arr[i-1].y), p.size)
...
def main():
    # Variables
    CLOCK = pygame.time.Clock(); still_out = 0; vel =1; move_snake = [[vel,0],'right']; crashed = False 
